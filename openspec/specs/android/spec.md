...
The CI/CD pipeline on GitHub Actions MUST automatically build the precompiled web assets, compile the Android APK, and attach the `.apk` package to the GitHub Release Assets upon publishing a release or pushing a version tag.

#### Scenario: GitHub Release APK asset upload
- **WHEN** a new GitHub Release is published or a tag matching `v*` is pushed
- **THEN** GitHub Actions runs validation tests, compiles the frontend bundle, builds the Android APK, and uploads the `.apk` to the release assets
