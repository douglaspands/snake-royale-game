...
### 4. Enable v1, v2 and v3 signing together
- **Decision**: Set `enableV1Signing`, `enableV2Signing` and `enableV3Signing` to true explicitly rather than relying on AGP defaults.
- **Rationale**: The shipped v1.5.3 APK carries v2 only, because AGP omits v1 when `minSdk >= 24`. That is correct per the platform spec, but OEM installers — Samsung's among them — have historically been stricter than AOSP about what they accept from a sideload, and a v1 signature costs only build time and a slightly larger archive. v3 is what makes future key rotation possible at all; adding it later, after the key is already in the field, is significantly harder. Declaring all three explicitly also pins the behavior against future AGP default changes.
- **Alternatives Considered**: v2+v3 only — a defensible reading of the platform requirements, but it leaves the one variable that plausibly differs between AOSP and Samsung's installer untested, in a change that exists specifically to fix a Samsung install failure.
